...
const theme: PdfTheme = {
  fontFamily: FONT_FAMILIES.sourceSans,
  accentColor: "#111827",
  textColor: "#111827",
  mutedColor: "#4b5563",
};

// Adds Publications, promotes Education — see index.tsx. react-pdf
// auto-paginates, so a long, multi-page CV needs no special handling.
export function AcademicCvPdf({ data, locale }: TemplateProps) {
  const pageSize = getLocaleProfile(locale).pageSize === "A4" ? "A4" : "LETTER";
  const { personal } = data;

  return (
    <Document>
      <Page size={pageSize} style={{ padding: 40, fontFamily: theme.fontFamily, color: theme.textColor, fontSize: 10 }}>
        ...
        <PdfPublicationsSection theme={theme} data={data} />
