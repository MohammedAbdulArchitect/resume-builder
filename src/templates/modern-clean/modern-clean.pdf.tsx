...
export function ModernCleanPdf({ data, locale }: TemplateProps) {
  ...
  return (
    <Document>
      <Page size={pageSize} style={{ padding: 44, fontFamily: theme.fontFamily, color: theme.textColor, fontSize: 10 }}>
        <View style={{ marginBottom: 20, borderBottomWidth: 2, borderBottomColor: theme.accentColor, paddingBottom: 10 }}>
          <Text style={{ fontSize: 22, fontWeight: 700 }}>{personal.fullName ?? "Your Name"}</Text>
          {personal.headline ? (
